...
class Vente:
    ...
    def ajouter_ligne(self, produit_id: ProduitId, quantite: int, prix_unitaire: Decimal) -> None:
        """
...
class Magasin:
    ...
    def peut_vendre(self, produit_id: ProduitId, quantite: int, stock_disponible: int) -> bool:
        """
...
class CommandeEcommerce:
    ...
    def valider_commande(self) -> None:
        ...
        if self._statut != StatutCommande.EN_ATTENTE:
            raise ValueError(f"Impossible de valider une commande en statut {self._statut.value}")
