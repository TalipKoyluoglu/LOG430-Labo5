...
class DjangoDemandeRepository(DemandeRepository):
    ...
    def get_by_id(self, demande_id: DemandeId) -> Optional[DemandeReapprovisionnement]:
        ...
        try:
            django_demande = DjangoDemandeReapprovisionnement.objects.get(id=str(demande_id))
            return self._mapper_vers_domaine(django_demande)
        except ObjectDoesNotExist:
            return None
    
    def get_demandes_by_statut(self, statut: StatutDemandeStock) -> List[DemandeReapprovisionnement]:
        """Récupère toutes les demandes avec un statut spécifique"""
        django_demandes = DjangoDemandeReapprovisionnement.objects.filter(statut=statut.value)
    ...
    def get_demandes_by_magasin(self, magasin_id: MagasinId) -> List[DemandeReapprovisionnement]:
    ...
    def get_demandes_by_produit(self, produit_id: ProduitId) -> List[DemandeReapprovisionnement]:
    ...
    def save(self, demande: DemandeReapprovisionnement) -> DemandeReapprovisionnement:
        ...
        try:
            ...
            django_demande = DjangoDemandeReapprovisionnement.objects.get(id=str(demande.demande_id))
    ...
    def exists_demande_en_attente(self, produit_id: ProduitId, magasin_id: MagasinId) -> bool:
    ...
    def _mapper_vers_domaine(self, django_demande: DjangoDemandeReapprovisionnement) -> DemandeReapprovisionnement:
