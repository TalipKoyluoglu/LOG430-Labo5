...
class HttpProduitService(ProduitService):
    ...
    def __init__(self, catalogue_base_url: str = "http://catalogue-service:8000"):
        ...
        logger.debug(f"HttpProduitService initialisé avec URL: {self.catalogue_base_url}")
    ...
    def produit_existe(self, produit_id: ProduitId) -> bool:
        ...
        try:
            ...
            logger.debug(f"Produit {produit_id} existe: {exists} (status: {response.status_code})")
            return exists
            
        except requests.RequestException as e:
            logger.warning(f"Erreur réseau lors de la vérification du produit {produit_id}: {e}")
    ...
    def get_nom_produit(self, produit_id: ProduitId) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Impossible de récupérer le nom du produit {produit_id} (status: {response.status_code})")
                return f"Produit {produit_id}"
                
        except requests.RequestException as e:
            logger.warning(f"Erreur réseau lors de la récupération du nom du produit {produit_id}: {e}")
            return f"Produit {produit_id}" 