...
from ...domain.entities import (
    StockCentral, StockLocal, DemandeReapprovisionnement, 
    StatutDemandeStock, TransfertStock, InventaireMagasin
)
from ...domain.value_objects import (
    ProduitId, MagasinId, Quantite, StockId, DemandeId
)
from ...domain.exceptions import (
    StockInsuffisantError, InventaireDomainError, 
    ProduitInexistantError, MagasinInexistantError
)
...
class GererStockUseCase:
    ...
    def consulter_stock(self, produit_id: str, magasin_id: Optional[str] = None) -> ConsulterStockResponse:
        """
        ...
        if magasin_id is None:
            ...
            stock = self.stock_repository.get_stock_local_by_produit_magasin(produit_id_vo, magasin_id_vo)
            if not stock:
                return ConsulterStockResponse(
                    produit_id=str(produit_id),
                    quantite=0,
    ...
    def lister_stocks_locaux_magasin(self, magasin_id: int) -> List[ConsulterStockResponse]:
        """
        ...
        for stock in tous_stocks_locaux:
            ...
            if magasin_id_str not in stocks_par_magasin:
                stocks_par_magasin[magasin_id_str] = {
                    ...
                    "nom_magasin": self.magasin_service.get_nom_magasin(stock.magasin_id),
                    "stocks": []
                }
    ...
    def _augmenter_stock_central(self, produit_id: ProduitId, quantite: Quantite) -> Dict[str, Any]:
    ...
    def _diminuer_stock_central(self, produit_id: ProduitId, quantite: Quantite) -> Dict[str, Any]:
        """Diminue le stock central avec validation métier"""
        stock = self.stock_repository.get_stock_central_by_produit(produit_id)
        
        if stock is None:
            raise StockInsuffisantError(f"Aucun stock central trouvé pour le produit {produit_id}")
    ...
    def _augmenter_stock_local(self, produit_id: ProduitId, magasin_id: MagasinId, quantite: Quantite) -> Dict[str, Any]:
        """Augmente le stock local avec gestion de création si nécessaire"""
        stock = self.stock_repository.get_stock_local_by_produit_magasin(produit_id, magasin_id)
    ...
    def _diminuer_stock_local(self, produit_id: ProduitId, magasin_id: MagasinId, quantite: Quantite) -> Dict[str, Any]:
        """Diminue le stock local avec validation métier"""
        stock = self.stock_repository.get_stock_local_by_produit_magasin(produit_id, magasin_id)
        
        if stock is None:
            raise StockInsuffisantError(f"Aucun stock local trouvé pour le produit {produit_id} au magasin {magasin_id}")
