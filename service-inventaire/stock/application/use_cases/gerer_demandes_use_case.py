...
from ...domain.value_objects import (
    ProduitId, MagasinId, Quantite, DemandeId
)
from ...domain.exceptions import (
    InventaireDomainError, ProduitInexistantError, 
...
class GererDemandesUseCase:
    ...
    def analyser_besoins_reapprovisionnement(self, magasin_id: int) -> Dict[str, Any]:
        ...
        stocks_faibles = self.stock_repository.get_stocks_locaux_faibles(magasin_id_vo, seuil=5)
        
        # Vérifier lesquels ont déjà une demande en attente
        produits_sans_demande = []
        for stock in stocks_faibles:
            if not self.demande_repository.exists_demande_en_attente(stock.produit_id, magasin_id_vo):
                produits_sans_demande.append({
                    ...
                    "quantite_suggeree": max(10 - int(stock.quantite), 5),  # Règle métier
        ...
        return {
            ...
            "recommandation": "Créer des demandes pour les produits listés" if produits_sans_demande else "Aucune action requise"
        }
    ...
    def approuver_demande(self, demande_id: str) -> DemandeResponse:
        ...
        stock_central = self.stock_repository.get_stock_central_by_produit(demande.produit_id)
    ...
    def _mapper_vers_response(self, demande: DemandeReapprovisionnement) -> DemandeResponse:
        ...
        return DemandeResponse(
            ...
            date_modification=demande.date_modification.isoformat() if demande.date_modification else None,
