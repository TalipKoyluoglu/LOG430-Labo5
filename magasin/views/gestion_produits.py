...
def uc4_modifier_produit(request, produit_id):
    ...
    try:
        ...
        if not produit_data.get('success', False):
            messages.error(request, f"Produit introuvable: {produit_data.get('error', 'Erreur inconnue')}")
        ...
        if request.method == "POST":
            ...
            messages.warning(request, "Modification de produit en cours d'implémentation dans l'API DDD")
            logger.warning(f"Tentative de modification produit {produit_id}: nom={nom}, prix={prix}")
...
def uc4_lister_produits(request):
    ...
    try:
        ...
        if not produits_data.get('success', False):
            ...
            messages.error(request, f"Erreur lors de la récupération des produits: {produits_data.get('error', 'Erreur inconnue')}")
            return render(request, "magasin/uc4_liste.html", {
                "produits": [],
...
def rechercher_produits(request):
    ...
    try:
        ...
        if not produits_data.get('success', False):
            messages.error(request, f"Erreur lors de la recherche: {produits_data.get('error', 'Erreur inconnue')}")
            return render(request, "magasin/uc4_recherche.html", {
                "produits": [],
                "criteres": criteres
            })
        ...
        logger.info(f"Recherche effectuée avec succès: {len(produits)} résultats pour {criteres}")
