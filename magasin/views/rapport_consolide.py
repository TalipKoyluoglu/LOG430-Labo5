...
def rapport_ventes(request):
    ...
    try:
        ...
        if isinstance(rapport_data, list):
            ...
            messages.error(request, f"Erreur lors de la génération du rapport: {rapport_data.get('error', 'Erreur inconnue')}")
            return render(request, "magasin/uc1_rapport.html", {
                "rapports": [],
        ...
        logger.info(f"Rapport consolidé généré avec succès: {len(rapports_formatted)} magasins")
...
def enregistrer_vente(request):
    ...
    try:
        ...
        if vente_result.get('success', False):
            messages.success(request, "Vente enregistrée avec succès via le service DDD")
            logger.info(f"Vente enregistrée: magasin={magasin_id}, produit={produit_id}, quantité={quantite}")
        else:
            error_msg = vente_result.get('error', 'Erreur inconnue lors de l\'enregistrement')
