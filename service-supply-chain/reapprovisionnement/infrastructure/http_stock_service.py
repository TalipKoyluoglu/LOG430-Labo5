...
class HttpStockService(StockService):
    ...
    def diminuer_stock_central(self, produit_id: ProduitId, quantite: Quantite) -> bool:
        ...
        try:
            url = f"{self.stock_service_url}/api/v1/stock/central/{produit_id}/decrease/"
            ...
            raise CommunicationServiceError(
                f"Impossible de diminuer le stock central: {e}",
                "service-inventaire"
            )
    
    def augmenter_stock_central(self, produit_id: ProduitId, quantite: Quantite) -> bool:
        """Augmente le stock central d'un produit (pour rollback)"""
        try:
            url = f"{self.stock_service_url}/api/v1/stock/central/{produit_id}/increase/"
            data = {"quantite": int(quantite)}
            
            response = requests.put(url, json=data, timeout=10)
            
            if response.status_code == 200:
                logger.info(f"Stock central augmenté (rollback): {produit_id} (+{quantite})")
                return True
            else:
            ...
            logger.error(f"Erreur de connexion: {e}")
            raise CommunicationServiceError(
                f"Impossible d'augmenter le stock central: {e}",
                "service-inventaire"
            )
    
    def augmenter_stock_local(self, produit_id: ProduitId, magasin_id: MagasinId, quantite: Quantite) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"Stock local augmenté: {produit_id} magasin {magasin_id} (+{quantite})")
                return True
            else:
            ...
            logger.error(f"Erreur de connexion: {e}")
            raise CommunicationServiceError(
                f"Impossible d'augmenter le stock local: {e}",
                "service-inventaire"
            )
    
    def diminuer_stock_local(self, produit_id: ProduitId, magasin_id: MagasinId, quantite: Quantite) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"Stock local diminué (rollback): {produit_id} magasin {magasin_id} (-{quantite})")
                return True
            else:
            ...
            logger.error(f"Erreur de connexion: {e}")
            raise CommunicationServiceError(
                f"Impossible de diminuer le stock local: {e}",
                "service-inventaire"
            ) 