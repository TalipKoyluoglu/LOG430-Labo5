...
from .value_objects import (
    DemandeId, ProduitId, MagasinId, Quantite, 
    MotifRejet, LogValidation
)
from .exceptions import (
    DemandeInvalideError, EtapeValidationError, 
    RollbackError, WorkflowError
)
...
class DemandeReapprovisionnement:
    ...
    def marquer_echec_validation(self, erreur: str) -> None:
        ...
        if self.statut != StatutDemande.EN_COURS_VALIDATION:
            raise WorkflowError("La validation doit être en cours pour marquer un échec")
...
class EtapeValidation:
    ...
    def marquer_succes(self) -> None:
        ...
        if not self.executee:
            raise EtapeValidationError("L'étape doit être exécutée avant d'être marquée comme réussie")
        self.succes = True
    
    def marquer_echec(self, erreur: str) -> None:
        """Marque l'étape comme échouée"""
        if not self.executee:
            raise EtapeValidationError("L'étape doit être exécutée avant d'être marquée comme échouée")
        self.succes = False
...
class WorkflowValidation:
    ...
    def finaliser_succes(self) -> None:
        ...
        if len(self.etapes_reussies) == len(self.etapes):
            ...
            raise WorkflowError("Toutes les étapes doivent être réussies pour finaliser")
