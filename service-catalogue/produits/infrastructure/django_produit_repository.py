...
from ..domain.value_objects import (
    ProduitId, CategorieId, NomProduit, PrixMonetaire, ReferenceSKU
)
...
class DjangoProduitRepository(ProduitRepository):
    ...
    def search_by_nom(self, terme: str) -> List[Produit]:
        ...
        ) | ProduitDjango.objects.filter(
            description__icontains=terme
        )
        return [self._to_domain_entity(p) for p in produits_django]
    ...
    def _update_django_from_domain(self, produit_django: ProduitDjango, produit: Produit) -> None:
        """
