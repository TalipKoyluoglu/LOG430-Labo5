...
class NomProduit:
    ...
    def __post_init__(self):
        ...
        if len(self.valeur.strip()) < 2:
            raise NomProduitInvalideError("Le nom du produit doit contenir au moins 2 caractères")
        
        if len(self.valeur) > 100:
            raise NomProduitInvalideError("Le nom du produit ne peut pas dépasser 100 caractères")
...
class PrixMonetaire:
    ...
    def comparer_avec(self, autre_prix: 'PrixMonetaire') -> str:
        ...
        if self.devise != autre_prix.devise:
            raise ValueError("Impossible de comparer des prix dans des devises différentes")
...
class ReferenceSKU:
    ...
    def __post_init__(self):
        ...
        if not re.match(pattern, self.code.upper()):
            raise SKUInvalideError("Le SKU doit avoir le format ABC1234 (3 lettres + 4 chiffres)")
...
class CommandeProduitSimple:
    ...
    def __post_init__(self):
        ...
        categories_valides = {"Informatique", "Boissons", "Confiserie", "Électronique", "Meubles", "Vêtements"}
        if self.categorie not in categories_valides:
            raise ValueError(f"Catégorie '{self.categorie}' non valide. Catégories acceptées: {', '.join(categories_valides)}")
