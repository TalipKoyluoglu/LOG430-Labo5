...
from ..domain.value_objects import (
    CritereRecherche, CommandeProduit, NomProduit, 
    PrixMonetaire, CategorieId, CommandeProduitSimple
)
from ..domain.exceptions import (
    CatalogueError, ProduitDejaExistantError, 
    CategorieInexistanteError, CriteresRechercheInvalidesError
)
...
class DDDCatalogueAPI(APIView):
    ...
    @swagger_auto_schema(
        ...
        **Architecture DDD:** Cette API orchestre le Use Case RechercherProduitsUseCase
        """,
        manual_parameters=[
            openapi.Parameter("nom", openapi.IN_QUERY, description="Recherche par nom", type=openapi.TYPE_STRING),
            openapi.Parameter("categorie_id", openapi.IN_QUERY, description="Filtrer par catégorie", type=openapi.TYPE_STRING),
            openapi.Parameter("prix_min", openapi.IN_QUERY, description="Prix minimum", type=openapi.TYPE_NUMBER),
            openapi.Parameter("prix_max", openapi.IN_QUERY, description="Prix maximum", type=openapi.TYPE_NUMBER),
            openapi.Parameter("actifs_seulement", openapi.IN_QUERY, description="Seulement produits actifs", type=openapi.TYPE_BOOLEAN, default=True),
        ],
        tags=["Catalogue DDD"],
    )
    ...
    @swagger_auto_schema(
        ...
        **Architecture DDD:** Cette API orchestre le Use Case AjouterProduitUseCase
        ...
        request_body=openapi.Schema(
            ...
            properties={
                'nom': openapi.Schema(type=openapi.TYPE_STRING, description='Nom du produit (2-100 caractères)'),
                'categorie': openapi.Schema(type=openapi.TYPE_STRING, description='Nom de catégorie (Informatique, Boissons, Confiserie, etc.)'),
                'prix': openapi.Schema(type=openapi.TYPE_NUMBER, description='Prix en euros (max 999999.99)'),
                'description': openapi.Schema(type=openapi.TYPE_STRING, description='Description du produit'),
            },
    ...
    def _build_criteres_recherche(self, query_params) -> CritereRecherche:
        ...
        if query_params.get('categorie_id'):
            try:
                ...
                raise CriteresRechercheInvalidesError("categorie_id doit être un UUID valide")
        ...
        if query_params.get('prix_min'):
            try:
                ...
                raise CriteresRechercheInvalidesError("prix_min doit être un nombre valide")
        ...
        if query_params.get('prix_max'):
            try:
                ...
                raise CriteresRechercheInvalidesError("prix_max doit être un nombre valide")
        
        actifs_seulement = query_params.get('actifs_seulement', 'true').lower() == 'true'
...
def get_produit_by_id(request, produit_id):
    ...
    try:
        ...
        return Response({
            ...
            'date_creation': produit.date_creation.isoformat() if produit.date_creation else None
        }, status=status.HTTP_200_OK)
