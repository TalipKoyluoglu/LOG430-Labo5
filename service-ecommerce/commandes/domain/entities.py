...
from .value_objects import CommandeEcommerce as CommandeEcommerceVO, LigneCommande, StatutCheckout, AdresseLivraison
...
class ProcessusCheckout:
    ...
    def valider_panier(self, lignes_panier: List[Dict]) -> None:
        ...
        if not lignes_panier:
            ...
            raise PanierVideError("Impossible de procéder au check-out avec un panier vide")
    ...
    def valider_stocks(self, stocks_disponibles: Dict[uuid.UUID, int]) -> None:
        ...
        if stocks_insuffisants:
            ...
            self._statut = StatutCheckout.ECHEC
            raise StockInsuffisantError("Stocks insuffisants pour certains produits", stocks_insuffisants)
    ...
    def finaliser_commande(self, commande_externe_id: uuid.UUID) -> None:
        ...
        if self._statut != StatutCheckout.STOCKS_VALIDES:
            raise CheckoutInvalideError(f"Impossible de finaliser: statut actuel {self._statut.value}")
    ...
    def peut_etre_finalise(self) -> bool:
        ...
        """
        return (
            self._statut == StatutCheckout.STOCKS_VALIDES and
            self._adresse_livraison is not None and
            len(self._lignes_commande) > 0 and
            len(self._erreurs) == 0
        )
    ...
    def obtenir_resume(self) -> Dict:
        ...
        return {
            ...
            "date_finalisation": self._date_finalisation.isoformat() if self._date_finalisation else None,
            "adresse_livraison": self._adresse_livraison.to_dict() if self._adresse_livraison else None,
            "commande_externe_id": str(self._commande_externe_id) if self._commande_externe_id else None,
            "lignes_commande": [
                {
            ...
            "calculs": {
                ...
                "nombre_articles": sum(ligne.quantite for ligne in self._lignes_commande),
    ...
    def _calculer_frais_livraison(self) -> None:
        ...
        if self._sous_total >= Decimal('100.00'):
            ...
            self._frais_livraison = Decimal('0.00')
        elif self._adresse_livraison and self._adresse_livraison.est_livraison_express():
            # Livraison express
...
class CommandeEcommerce:
    ...
    def definir_details_financiers(self, sous_total: Decimal, frais_livraison: Decimal) -> None:
