...
def checkout_ecommerce(request, client_id):
    ...
    try:
        ...
        champs_manquants = [
            champ for champ in champs_requis 
            if not adresse_data.get(champ, '').strip()
        ]
        ...
        try:
            from .application.use_cases.checkout_ecommerce_use_case import CheckoutEcommerceUseCase
...
def verifier_prerequis_checkout(request, client_id):
    ...
    try:
        ...
        try:
            ...
            panier_validation = panier_service.valider_panier_pour_checkout(str(client_uuid))
            ...
            if panier_validation['valide']:
                produits_panier = panier_service.obtenir_produits_pour_stock(str(client_uuid))
                if produits_panier:
                    stocks_validation = stock_service.valider_stocks_suffisants(produits_panier)
            
            # Construction de la réponse
            peut_commander = panier_validation['valide'] and stocks_validation['tous_suffisants']
            ...
            prerequis = {
                ...
                'details': {
                    ...
                    'panier': {
                        'existe': panier_validation.get('panier', {}).get('panier_existe', False),
                        'nombre_articles': panier_validation.get('resume', {}).get('nombre_articles', 0),
                        'total': panier_validation.get('resume', {}).get('total', 0),
                        'raison_invalide': panier_validation.get('raison') if not panier_validation['valide'] else None
                    },
                    'stocks': {
                        'tous_disponibles': stocks_validation['tous_suffisants'],
                        'produits_ok': stocks_validation.get('resume', {}).get('produits_ok', 0),
                        'produits_ko': stocks_validation.get('resume', {}).get('produits_ko', 0),
                        'stocks_insuffisants': stocks_validation.get('stocks_insuffisants', [])
                    }
                }
            }
...
def historique_commandes_client(request, client_id):
    ...
    try:
        ...
        try:
            from .application.use_cases.lister_commandes_client_use_case import ListerCommandesClientUseCase
            from .infrastructure.django_commande_ecommerce_repository import DjangoCommandeEcommerceRepository
            ...
            logger.info(f"Historique récupéré pour client {client_id}: {historique['statistiques']['nombre_commandes']} commandes")
        ...
        logger.error(f"Erreur lors de la récupération de l'historique pour client {client_id}: {str(e)}")
        return Response({
            'error': 'Erreur interne du serveur',
            'details': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR) 