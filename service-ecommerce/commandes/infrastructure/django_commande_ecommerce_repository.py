...
from ..application.repositories.commande_ecommerce_repository import CommandeEcommerceRepository
...
from ..models import CommandeEcommerce as CommandeEcommerceDjango, LigneCommandeEcommerce as LigneCommandeEcommerceDjango
...
class DjangoCommandeEcommerceRepository(CommandeEcommerceRepository):
        ...
        try:
            ...
            django_commande, created = CommandeEcommerceDjango.objects.get_or_create(
                ...
                defaults={
                    ...
                    'nom_destinataire': commande.adresse_livraison.nom_destinataire if commande.adresse_livraison else '',
                    'rue': commande.adresse_livraison.rue if commande.adresse_livraison else '',
                    'ville': commande.adresse_livraison.ville if commande.adresse_livraison else '',
                    'code_postal': commande.adresse_livraison.code_postal if commande.adresse_livraison else '',
                    'province': commande.adresse_livraison.province if commande.adresse_livraison else '',
                    'pays': commande.adresse_livraison.pays if commande.adresse_livraison else 'Canada',
                    'instructions_livraison': commande.adresse_livraison.instructions_livraison if commande.adresse_livraison else None,
                    'livraison_express': commande.adresse_livraison.livraison_express if commande.adresse_livraison else False,
    ...
    def get_by_client_id(self, client_id: UUID, limit: int = 50) -> List[CommandeEcommerceDomain]:
    ...
    def get_by_checkout_id(self, checkout_id: UUID) -> Optional[CommandeEcommerceDomain]:
        """Récupère une commande par son checkout ID"""
        try:
            django_commande = CommandeEcommerceDjango.objects.get(checkout_id=checkout_id)
            return self._mapper_vers_domaine(django_commande)
        except CommandeEcommerceDjango.DoesNotExist:
            return None
    
    def get_recent_commandes(self, days: int = 30, limit: int = 100) -> List[CommandeEcommerceDomain]:
    ...
    def _save_lignes_commande(self, django_commande: CommandeEcommerceDjango, lignes: List[LigneCommande]) -> None:
    ...
    def _mapper_vers_domaine(self, django_commande: CommandeEcommerceDjango) -> CommandeEcommerceDomain:
