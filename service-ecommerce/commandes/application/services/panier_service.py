...
class PanierService:
        ...
        try:
            ...
            if resultat.get('panier_existe', False):
                return {
                    ...
                    'client_id': client_id,
                    'nombre_articles': resultat.get('resume', {}).get('nombre_articles', 0),
                    'nombre_produits': resultat.get('resume', {}).get('nombre_produits_differents', 0),
                    ...
                    'produits': [
                        {
                            ...
                            'prix_total': produit['prix_ligne']  # Note: dans VoirPanierUseCase c'est 'prix_ligne'
                        }
    ...
    def vider_panier_apres_commande(self, client_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if resultat['success']:
                ...
                logger.warning(f"Échec vidage panier client {client_id}: {resultat.get('message')}")
                return {
                    'success': False,
    ...
    def obtenir_produits_pour_stock(self, client_id: str) -> list:
        ...
        if not panier_info['success'] or not panier_info['panier_existe'] or panier_info['panier_vide']:
            return []
