...
from ...domain.exceptions import ServiceExterneIndisponibleError, CreationCommandeEchecError
...
class CommandesService:
    ...
    def __init__(self):
        self.commandes_url = settings.SERVICES.get('COMMANDES', 'http://commandes-service:8003')
    
    def creer_commande_ecommerce(self, commande_ecommerce: CommandeEcommerce) -> uuid.UUID:
        """
        ...
        try:
            ...
            magasin_ecommerce_id = "11111111-1111-1111-1111-111111111111"  # Magasin virtuel
            ...
            for ligne in commande_ecommerce.lignes_commande:
                ...
                logger.debug(f"Vente créée pour produit {ligne.produit_id}: {commande_id}")
            ...
            commande_principale_id = commandes_creees[0] if commandes_creees else uuid.uuid4()
            
            logger.info(f"Commande e-commerce créée avec succès: {commande_principale_id}")
            logger.info(f"Total de {len(commandes_creees)} ventes créées dans le service-commandes")
            
            return commande_principale_id
            
        except Exception as e:
            logger.error(f"Erreur lors de la création de commande e-commerce: {str(e)}")
            raise CreationCommandeEchecError(f"Impossible de créer la commande: {str(e)}")
