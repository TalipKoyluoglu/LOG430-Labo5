    ...
    def execute(self, client_id: str, limite: int = 50) -> Dict[str, Any]:
        ...
        for commande in commandes_domaine:
            commande_dict = {
                ...
                "adresse_livraison": commande.adresse_livraison.to_dict() if commande.adresse_livraison else None,
        ...
        commande_moyenne = total_depense / nombre_commandes if nombre_commandes > 0 else 0.0
        derniere_commande = commandes_formatees[0]["date_commande"] if commandes_formatees else None
        
        logger.debug(f"Historique récupéré: {nombre_commandes} commandes, total {total_depense}€")
