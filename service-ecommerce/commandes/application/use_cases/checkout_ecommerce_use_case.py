...
from ...domain.value_objects import DemandeCheckout, CommandeEcommerce as CommandeEcommerceVO, LigneCommande, AdresseLivraison
from ...domain.entities import CommandeEcommerce as CommandeEcommerceDomain
from ...domain.exceptions import (
    CheckoutError, PanierVideError, StockInsuffisantError, 
    AdresseInvalideError, ServiceExterneIndisponibleError
)
...
class CheckoutEcommerceUseCase:
    ...
    def execute(self, demande_checkout: DemandeCheckout) -> Dict[str, Any]:
        ...
        try:
            ...
            frais_livraison = self._calculer_frais_livraison(
                sous_total, 
                demande_checkout.adresse_livraison.livraison_express
            )
            ...
            if not vidage_result['success']:
                logger.warning(f"Échec vidage panier {client_id}: {vidage_result.get('error')}")
            ...
            logger.info(f"Checkout {checkout_id} terminé avec succès - Commande {commande_id}")
    ...
    def _verifier_stocks_centraux(self, panier_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        produits_panier = self.panier_service.obtenir_produits_pour_stock(panier_info['client_id'])
        return self.stock_service.valider_stocks_suffisants(produits_panier)
    
    def _calculer_frais_livraison(self, sous_total: Decimal, livraison_express: bool) -> Decimal:
        """
    ...
    def _decrémenter_stocks_centraux(self, panier_info: Dict[str, Any]) -> None:
        ...
        for produit in panier_info['produits']:
            ...
            if not result['success']:
                error_msg = f"Échec décrémentation stock {produit_id}: {result.get('error')}"
                logger.error(error_msg)
                raise ServiceExterneIndisponibleError('inventaire', error_msg)
            
            logger.info(f"Stock décrementé: {produit['nom_produit']} -{quantite} = {result['nouvelle_quantite']}")
