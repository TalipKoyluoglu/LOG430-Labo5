...
class AjouterProduitPanierUseCase:
    ...
    def execute(
        ...
        logger.info(f"Ajout produit {commande_ajout.produit_id} au panier client {client_id}")
        
        # 1. Vérifier que le produit existe et est actif (service-catalogue)
        try:
            produit_info = self._catalogue_service.obtenir_produit(commande_ajout.produit_id)
            if not produit_info.get('actif', False):
                raise ProduitInexistantError(f"Produit {commande_ajout.produit_id} n'est pas actif")
        except ProduitInexistantError:
            logger.warning(f"Tentative d'ajout d'un produit inexistant: {commande_ajout.produit_id}")
            raise
        ...
        if not self._stock_service.verifier_stock_disponible(
            ...
            stock_disponible = self._stock_service.obtenir_stock_disponible(commande_ajout.produit_id)
            logger.warning(f"Stock insuffisant pour produit {commande_ajout.produit_id}: "
        ...
        logger.info(f"Produit {commande_ajout.produit_id} ajouté avec succès au panier {panier.id}")
