...
class StockService:
    ...
    def __init__(self):
        self.inventaire_url = settings.SERVICES.get('INVENTAIRE', 'http://inventaire-service:8002')
    
    def verifier_stock_disponible(self, produit_id: uuid.UUID, quantite_demandee: int) -> bool:
        """
        ...
        try:
            ...
            url = f"{self.inventaire_url}/api/ddd/inventaire/stock-central/{produit_id}/"
            
            logger.debug(f"Vérification stock pour produit {produit_id}, quantité: {quantite_demandee}")
    ...
    def reserver_stock(self, produit_id: uuid.UUID, quantite: int) -> bool:
        ...
        try:
            ...
            data = {
                'quantite': quantite,
                'type_reservation': 'ecommerce'
            }
            
            logger.debug(f"Réservation stock: {quantite} unités du produit {produit_id}")
