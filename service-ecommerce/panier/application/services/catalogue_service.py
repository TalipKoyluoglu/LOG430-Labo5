...
class CatalogueService:
    ...
    def __init__(self):
        self.catalogue_url = settings.SERVICES.get('CATALOGUE', 'http://catalogue-service:8000')
    ...
    def obtenir_produit(self, produit_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                raise ProduitInexistantError(f"Produit {produit_id} n'existe pas dans le catalogue")
            ...
            raise ProduitInexistantError(f"Impossible de vérifier l'existence du produit {produit_id}")
        except Exception as e:
            logger.error(f"Erreur lors de la récupération du produit {produit_id}: {str(e)}")
            raise ProduitInexistantError(f"Erreur lors de la récupération du produit {produit_id}")
