...
class DjangoPanierRepository(PanierRepository):
    ...
    def save(self, panier: Panier) -> None:
        ...
        with transaction.atomic():
            ...
            logger.debug(f"Panier {panier.id} sauvegardé avec {len(panier.produits)} produits")
    ...
    def get_by_client_id(self, client_id: uuid.UUID) -> Optional[Panier]:
        ...
        """
        try:
            panier_model = PanierModel.objects.filter(
                client_id=client_id
    ...
    def get_all_by_client(self, client_id: uuid.UUID) -> List[Panier]:
        ...
        """
        try:
            paniers_models = PanierModel.objects.filter(
                client_id=client_id
